...
import { match as originalMatch } from 'oxide.ts';

export { Default, Err, Fn, None, Ok, Option, Result, Some } from 'oxide.ts';

type LowercaseMatchPatterns<T, E> = {
  ok: (val: T) => void;
  err: (val: E) => void;
};
 ...
 * ```
 */
export function match<T, E>(val: Result<T, E>, patterns: LowercaseMatchPatterns<T, E>) {
  return originalMatch(val, {
    // biome-ignore lint/style/useNamingConvention: Fix to use camelCase in future
    Ok: patterns.ok,
    // biome-ignore lint/style/useNamingConvention: Fix to use camelCase in future
    Err: patterns.err,
  });
